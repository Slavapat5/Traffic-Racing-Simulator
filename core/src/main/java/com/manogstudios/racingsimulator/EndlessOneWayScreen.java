...
public class EndlessOneWayScreen implements Screen {
    ...
    private void showGameOverOverlay(int cashEarned,
        ...
        panel.add(scoreBox).width(520).fillX().padBottom(15).row();
        ...
        panel.add(cashBox).width(520).fillX().padBottom(22).row();
    ...
    private void onCrash() {
        ...
        Runnable showGameOverSummary = () -> {
            cashLabel.setText("$" + formatCash(CashManager.getCash()));
            showGameOverOverlay(cashEarned, distMeters, bestScore, newBestScore, fastLanePoints);
        };
